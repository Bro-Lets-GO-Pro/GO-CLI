class Board{
	...
	void move_calculator(int, vector<int>&);	//Takes the side as an argument and an vector. The vector when passed into it is supposed to be empty but after the function call is complete it would contain integers(all posible position it can go to)
...
void Board::move_maker(bool side, int position){
	
}

int Board::move_calculator(bool side, vector<int>& moves){
